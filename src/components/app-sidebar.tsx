...
export function AppSidebar() {
  ...
  const getNavCls = ({ isActive }: { isActive: boolean }) =>
    isActive ? "text-white font-semibold" : "transition-all duration-300 text-muted-foreground hover:bg-transparent hover:text-foreground"
                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={isActive(item.url)}>
                    {item.external ? (
                      ...
                      >
                        <item.icon className="h-4 w-4 opacity-50 group-hover/menu:opacity-100 group-hover/menu:scale-110 transition-all duration-300" />
                        {!isCollapsed && (
                          <div className="flex items-center justify-between w-full">
                            <span className="font-medium text-foreground">{item.title}</span>
                            <ExternalLink className="h-3 w-3 opacity-50 group-hover/menu:opacity-100 transition-all duration-300 group-hover/menu:scale-110" />
                          </div>
                        )}
                      </a>
                    ) : (
