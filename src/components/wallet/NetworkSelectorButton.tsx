    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div>
          <GradientBorderButton className="w-auto justify-center">
            <img src={currentNetwork.logo} alt={currentNetwork.name + ' logo'} loading="lazy" className="h-6 w-auto object-contain shrink-0" />
      ...
      <DropdownMenuContent align="end" className="w-48 bg-[hsl(var(--menu-surface))] border-white/20 z-50">
        {networks.map((network) => (
          ...
          >
            <img src={network.logo} alt={network.name + ' logo'} loading="lazy" className="h-6 w-auto object-contain mr-2 shrink-0" />
            {network.name}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
